...
- `reports/monthly/YYYY-MM.md` に保存。記事IDは `MR-YYYYMM-NN`。末尾に # Sources(参照した自分のファイル一覧)、# Review、# Feedback
- 保存前に source-validator(形式チェック)を実行する
- **独立レビュー(裏取り+校閲)を必ず実行する**(EV-20260713-01 で承認、evolution/changelog.md 参照)。外部情報を引用した箇所は執筆者と別の Agent(general-purpose)で出典照合(反証姿勢・判定・根拠URL)し、あわせて校閲(誤字・見出し・output-preferences 整合)を実施、指摘を反映して修正する。本人の記録のみで構成される内省部分は校閲を中心に確認する
